...
RUN apt-get install -qqy unzip && \
    wget -c http://downloads.sourceforge.net/project/geoserver/GeoServer/2.11.0/geoserver-2.11.0-bin.zip \
         -O /tmp/geoserver-2.11.0-bin.zip && \
    unzip /tmp/geoserver-2.11.0-bin.zip -d /opt && \
    cd /opt && \
    ln -s geoserver-2.11.0 geoserver
